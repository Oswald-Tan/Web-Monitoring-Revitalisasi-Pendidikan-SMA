...
const Layout = () => {
  ...
  useEffect(() => {
    const fetchSchoolData = async () => {
      try {
        ...
        setName(school.name || "");
        setKabupaten(school.kabupaten || "");
        setLocation(school.location || "");
        setNilaiBanper(
          school.nilaiBanper
            ? formatCurrency(school.nilaiBanper.toString())
            : ""
        );
        setDurasi(school.durasi ? school.durasi.toString() : "");
        setStartDate(formatDateForInput(school.startDate));
        setFinishDate(formatDateForInput(school.finishDate));
        setFacilitator(school.facilitator_id || "");
      } catch (error) {
        console.error("Gagal mengambil data sekolah:", error);
        Swal.fire("Error", "Gagal mengambil data sekolah", "error");
        navigate("/sekolah/daftar-sekolah");
      } finally {
        setIsFetching(false);
      }
    };
  ...
  const updateSchool = async (e) => {
    ...
    try {
      await axios.put(`${API_URL}/schools/${id}`, {
        name,
        kabupaten,
        location,
      ...
      Swal.fire("Success", "Sekolah berhasil diperbarui", "success");
    } catch (error) {
      if (error.response) {
    ...
    <div>
      <div className="space-y-6 w-full">
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
          <div className="p-8">
            <form onSubmit={updateSchool} className="space-y-6">
              {/* Error Message */}
              {msg && (
              ...
              <div className="grid gap-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <InputField
                    ...
                    value={nilaiBanper}
                    onChange={(e) =>
                      setNilaiBanper(formatCurrency(e.target.value))
                    }
              ...
              <div className="pt-6 border-t border-gray-200 flex gap-3">
                <button
                  type="button"
                  onClick={() => navigate("/sekolah/daftar-sekolah")}
                  className="px-6 py-3 text-sm bg-gray-200 text-gray-700 font-medium rounded-xl shadow-sm hover:shadow-md focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-all duration-200"
                >
                  Cancel
                </button>
                <button
