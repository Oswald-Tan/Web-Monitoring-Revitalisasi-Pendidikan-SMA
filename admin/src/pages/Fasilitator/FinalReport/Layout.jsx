...
const Layout = () => {
  ...
  const generateReport = async () => {
    if (!selectedSchool || !periodStart || !periodEnd) {
      Swal.fire("Peringatan", "Pilih sekolah dan periode terlebih dahulu", "warning");
      return;
    }
  ...
  const downloadPDF = async (id) => {
    try {
      ...
      if (report) {
        link.download = `laporan-akhir-${report.School.name}-${new Date(report.periodStart).toISOString().split('T')[0]}.pdf`;
      } else {
  ...
  const exportAllPDF = async () => {
    try {
      const response = await axios.get(`${API_URL}/final-reports/export/pdf`, {
        responseType: "blob"
      });
      ...
      const link = document.createElement("a");
      link.href = url;
      link.download = `laporan-akhir-semua-${new Date().toISOString().split('T')[0]}.pdf`;
      ...
      Swal.fire("Error", "Gagal mengekspor semua PDF", "error");
    }
  };
  ...
  const getStatusBadge = (status) => {
    const statusConfig = {
      ...
      published: { color: "bg-purple-100 text-purple-800", text: "Dipublikasi" }
    };
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  <th className="text-left py-4 px-6 text-sm font-semibold text-gray-900">Sekolah</th>
                  <th className="text-left py-4 px-6 text-sm font-semibold text-gray-900">Periode</th>
                  <th className="text-left py-4 px-6 text-sm font-semibold text-gray-900">Status</th>
                  <th className="text-left py-4 px-6 text-sm font-semibold text-gray-900">Dibuat Oleh</th>
                  <th className="text-left py-4 px-6 text-sm font-semibold text-gray-900">Tanggal Dibuat</th>
                  <th className="text-left py-4 px-6 text-sm font-semibold text-gray-900">Aksi</th>
                </tr>
              </thead>
              <tbody>
                {reports.map((report) => (
                  <tr key={report.id} className="border-b border-gray-200 hover:bg-gray-50">
                    <td className="py-4 px-6">
                      <div className="text-sm font-medium text-gray-900">{report.School.name}</div>
                      <div className="text-sm text-gray-500">{report.School.kabupaten}</div>
                    </td>
                    <td className="py-4 px-6 text-sm">
                      {formatDate(report.periodStart)} - {formatDate(report.periodEnd)}
                    </td>
                    <td className="py-4 px-6">
                      <div dangerouslySetInnerHTML={{ __html: getStatusBadge(report.status) }} />
                    </td>
                    <td className="py-4 px-6 text-sm">{report.generator?.name || "-"}</td>
                    <td className="py-4 px-6 text-sm">{formatDate(report.createdAt)}</td>
                    <td className="py-4 px-6">
                      <div className="flex space-x-2">
                        <button
